...
{
    /// <summary>
    /// Compuerta OR. Tiene al menos dos entradas y si una de sus entradas es true, entonces su salida será true.
    /// </summary>
    ...
    public class OrGate : IGate
    {
        ...
        #region MÉTODOS
        /// <summary>
        /// Método constructor de compuertas OR.
        /// </summary>
        ...
        public bool CalculateInput()
        {
            ...
            {
                // Se supone que la salida es false.
                bool result = false;

                // Primero lo más fácil de revisar, es decir, las variables simples.
                foreach (SimpleVariable simpleInput in simpleInputs)
                {
                    if (simpleInput.Value == true)
                    {
                        return true;
                    }
                }

                // Si todas las variables simples son false, entonces va a revisar las salidas de las compuertas antecesoras.
                foreach (IGate gate in inputGates)
                {
                    if (gate.CalculateInput() == true)
                    {
                        return true;
                    }
                }
